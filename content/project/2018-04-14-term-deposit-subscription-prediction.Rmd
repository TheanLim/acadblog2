...
```
About 80% of the predictor `poutcome`, 50% of predictor `contact`, 5% of predictor `education` and 1% of predictor `job` have missing data (see Figure \@ref(fig:NAplot)). There will be `r length(unique (unlist (lapply (full_dat, function (x) which (is.na (x))))))` rows that had to be deleted if rowwise NA deletion was executed. The other way is to remove entirely those variables that contains NA including `poutcome`, `contact`, `education` and `job`. However, this is not desirable because we dont know whether those variables are important or not. In short, deletion is not an ideal way.
...
```
The error of misclassification was `r err_tree`. However, this is the traning error rate and it could be biased. Cross-validation was used to choose the best number of terminal nodes and the decision was pruned.  
...
- Contact the prospective "subscribers" at the end of year quarters.
